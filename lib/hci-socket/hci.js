...
//const OCF_LE_CONN_PARAM_REQ = 0x000f;
...
//const LE_CONN_PARAM_REQ_CMD = OCF_LE_START_ENCRYPTION | OGF_LE_CTL << 10;
...
class Hci extends EventEmitter {
	constructor() {
		...
		this.scanParams = {
			type: 1,		// 0 -> passive, 1 -> active
			interval: 100,		// [ms]
			window: 100,		// [ms]
			ownAddress: 0,		// 0 -> public, 1 -> random
			filter: 0		// 0 -> all event types
		};
	}

	static get STATUS_MAPPER() {
	...
	setScanParameters() {
		...
		// data
		cmd.writeUInt8(this.scanParams.type, 4);		// type: 0 -> passive, 1 -> active
		cmd.writeUInt16LE(Math.floor(this.scanParams.interval / 0.625), 5); // internal, ms * 1.6
		cmd.writeUInt16LE(Math.floor(this.scanParams.window / 0.625), 7); // window, ms * 1.6
		cmd.writeUInt8(this.scanParams.ownAddress, 9);		// own address type: 0 -> public, 1 -> random
		cmd.writeUInt8(this.scanParams.filter, 10);		// filter: 0 -> all event types
	...
	createLeConn(address, addressType) {
		...
		cmd.writeUInt16LE(Math.floor(this.connParams.minInterval / 1.25), 17);	// min interval
		cmd.writeUInt16LE(Math.floor(this.connParams.maxInterval / 1.25), 19);	// max interval
		cmd.writeUInt16LE(this.connParams.latency, 21);				// latency
		cmd.writeUInt16LE(Math.floor(this.connParams.supervisionTimeout / 10), 23); // supervision timeout
		cmd.writeUInt16LE(this.connParams.minCeLen, 25);			// min ce length
		cmd.writeUInt16LE(this.connParams.maxCeLen, 27);			// max ce length
	...
	connParamReqLe(handle, minInterval, maxInterval, latency, supervisionTimeout) {
		const cmd = new Buffer(18);
		const offsets = [0, 0.5, 0xffff, 0xffff, 0xffff, 0xff];
		// header
		cmd.writeUInt8(HCI_COMMAND_PKT, 0);
		cmd.writeUInt16LE(LE_CONN_PARAM_REQ_CMD, 1);	//HCI_BLE_UPD_LL_CONN_PARAMS

		// length
		cmd.writeUInt8(0x19, 3);						// HCIC_PARAM_SIZE_BLE_UPD_LL_CONN_PARAMS
		...
		cmd.writeUInt8(0, 14);	//PreferredPeriodicity: 0
		cmd.writeUInt16LE(0, 15);	//ReferenceConnEventCount: <any value that is less than 32767 connection events in the future>

		for (let i=0; i<=5; i++) {
			let val = offsets[i];
			if (val <= 1) {
				val *= Math.floor(minInterval / 1.25);
			}
			cmd.writeUInt16LE(val, 17 + i*2);
		}
/*
		cmd.writeUInt16LE(0, 17);	//Offset0: 0x0003
		cmd.writeUInt16LE(Math.floor(minInterval / 1.25)/2, 19);	//Offset1: 0xFFFF
		cmd.writeUInt16LE(0xFFFF, 21);	//Offset2: 0xFFFF
		cmd.writeUInt16LE(0xFFFF, 23);	//Offset3: 0xFFFF
		cmd.writeUInt16LE(0xFFFF, 25);	//Offset4: 0xFFFF
		cmd.writeUInt16LE(0xFFFF, 27);	//Offset5: 0xFFFF	
		*/
...
console.log("onSocketData: encryptChange");
