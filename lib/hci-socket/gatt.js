			...
			if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
				...
				for (let i = 0; i < num; i++) {
					includedServices.push({
						startHandle: data.readUInt16LE(2 + i * type + 2),	// FIXME?
	...
	discoverDescriptors(handle) {
		...
		const callback = (data) => {
			...
			if (opcode === ATT_OP_FIND_INFO_RESP) {
				...
				for (let i = 0; i < num; i++) {
					descriptors.push({
						startHandle: data.readUInt16LE(2 + i * 4 + 0),
		...
		if (1 && characteristic.valueHandle === characteristic.endHandle) {
			...
			const callback = (data) => {
				...
				if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
					...
					if (data.length === this._mtu) {
						...
						this.emit('read', this._address, handle, readData);
		...
		const promise = new Promise((resolve, reject) => {
			...
			if (withoutResponse) {
				this._queueCommand(this.writeRequest(characteristic.valueHandle, data, true), null, () => {
					this.emit('write', this._address, handle);
					resolve(this._address, handle);
				});
			} else if (data.length + 3 > this._mtu) {
				return this.longWrite(handle, data, withoutResponse);
			} else {
