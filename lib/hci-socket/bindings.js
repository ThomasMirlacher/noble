...
class NobleBindings extends EventEmitter {
	...
	onLeConnComplete(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
		...
		if (status === 0) {
			...
			this._gatts[peripheralUuid] = this._gatts[handle] = gatt;
			this._signalings[peripheralUuid] = this._signalings[handle] = signaling;
	...
	onDisconnComplete(handle, reason) {
		...
		if (peripheralUuid) {
			...
			delete this._gatts[peripheralUuid];
			delete this._gatts[handle];
			delete this._signalings[peripheralUuid];
	...
	discoverServices(peripheralUuid, uuids) {
		const gatt = this._gatts[peripheralUuid];
